...
@section('content')
    ...
    <div class="container">
        ...
        <form method="POST" id="storeDish" action="{{ route('admin.dishes.update', $dish) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" name="description"
                    value="{{ old('description', $dish->description) }}" min="20" max="1000" required>
                @error('description')
                    <div class="form-text text-danger">{{ $message }}</div>
                @enderror
            </div>
